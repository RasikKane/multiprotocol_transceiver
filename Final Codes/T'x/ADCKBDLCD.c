...
up:
	IO0DIR=0x2000f000;         	  //set rows as output
...
void LcdInit (void)
{
	...
	IO1DIR |= 0x03400000  ;		// Configure P1.24(EN) and P1.25(RS) as Output
...
up:
	...
	PINSEL2 |=0x00000000;
//	printf("checked");
	 IO1DIR |=0x000f0000;         	  //set rows as output
	
	//printf("\n no key pressed");
	IO1SET =0x00f00000;
	IO1CLR =0x000f0000;		//all rows 0
		//printf("checked");

	delay();
	a=P1 & 0x00f00000;
//	printf("\nchecked %#x",a);
	 if(a==0x00f00000)		//is key pressed?
		goto up;
...
//		printf("\nchecked");//while ((P0&0x00f00000))
	...
	IO1CLR=0x00020000;		//row_1 set to 0
	IO1SET=0x000d0000;
	//delay();
	a=P1 & 0x00f00000;
	 if(a!=0x00f00000)
...
//while (!(P0&0x00f00000))
//		goto row_2;
	IO1CLR=0x00080000;		//row_3 set to 0
	IO1SET=0x00070000;
	//delay();
	a=P1 & 0x00f00000;
	 if(a!=0x00f00000)
